<template>
  ...
  >
    <header>
        <h3>当前授权的角色是：【{{title}}】</h3>
    </header>
    <div style="display: flex;justify-content: space-between;margin-top: 20px;">
        <el-input style="width: 60%;" v-model="keyword" placeholder="筛选" maxlength="20"></el-input>
        <el-button type="primary" @click="handleSubmitMenusToRole" icon="check" style="padding:8px 16px!important;">确定</el-button>
    </div>
    <div class="treebox">
        <el-tree
            ref="treeRef"
...
<script setup>
    ...
    const handleOpen = async (role) => {
        ...
        // 查询当前角色已经授权的菜单
        const resp =  await SelectRoleMenus(role.id)
        if(resp.data && resp.data?.length > 0){
            var menuIdArr = resp.data.map(c=>c.id)
            // 然后把授权的菜单，全部选中
    ...
    const handleSubmitMenusToRole = async () => {
        ...
        await SaveRoleMenus({"roleId":roleId,"menuIds":menuIds})

<template>
  <el-dialog  :close-on-click-modal="false" @close="handleClose" v-model="dialogFormVisible" :title="title"
     width="960px">
    <el-form v-loading="formLoading" ref="formRef" 
    label-width="100" 
    :model="form" style="padding: 30px" :rules="rules">
      <el-form-item label="父级菜单：" required prop="parentId">
        <el-select v-model="form.parentId" style="width: 100%;" placeholder="请选择菜单" 
...
const handleLoadRoot = async () => {
  ...
  parentMenus.value = parentMenus.value.concat(resp.data)
}

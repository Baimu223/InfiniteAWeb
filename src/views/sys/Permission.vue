      ...
      <home-page-header>
        ...
        >
          ...
          <el-table-column prop="code" label="请求方式" align="center" >
            <template #default="{row}">
              <el-select v-model="row.method" style="width: 100%;" @change="handleChange(row,'method')" placeholder="请选择请求方式" >
                <el-option
                  v-for="item in [{id:'GET',title:'GET'},{id:'POST',title:'POST'}]"
                  :key="item.id"
                  :label="item.title"
                  :value="item.id"
                />
          ...
          <el-table-column fixed="right" align="left" label="操作" width="350">
            <template #default="{row,$index}">
              <el-button text icon="edit" @click="handleAddChild(row)" v-if="row.parentId == 0"  type="primary">添加子权限</el-button>
